...
# /home/niharika/Desktop/Hydrological_Modelling/Final_Hydro/Model_Pickels/SVR_LINEAR.sav
svrRbf=joblib.load('/home/niharika/Desktop/4_2/RR/Hydrological_Modelling/Final_Hydro/Model_Codes/model_files/SVR_RBF.sav','r')
svrLin=joblib.load('/home/niharika/Desktop/4_2/RR/Hydrological_Modelling/Final_Hydro/Model_Codes/model_files/SVR_LINEAR.sav','r')
svrPoly=joblib.load('/home/niharika/Desktop/4_2/RR/Hydrological_Modelling/Final_Hydro/Model_Codes/model_files/SVR_POLYNOMIAL.sav','r')


#File specific 
from scalers import sc_X,sc_y

class SupportVectorMachine(SelectPage):
    ...
    def __init__(self, parent, controller):
        ...
        #Label 
        self.kernel_label=ttk.Label(self, text = "Select Kernel :", font =Vars.LABEL_FONT)
        ...
        self.label_name=tk.Label(self, text = "Support Vector Machine", font = Vars.LARGE_FONT,bg='cyan')
    ...
    def _calculate_csv_output(self):
        ...
        if inputFilename == "" or outputFilename == "":
            ...
            if(self.kernel_variable.get()=='RBF'):
                y_pred=self.scalery.inverse_transform((svrRbf.predict(self.scalerx.transform(data))))
            elif(self.kernel_variable.get()=='Linear'):
               y_pred=self.scalery.inverse_transform((svrLin.predict(self.scalerx.transform(data))))
            elif(self.kernel_variable.get()=='Polynomial'):
               y_pred=self.scalery.inverse_transform((svrPoly.predict(self.scalerx.transform(data))))
            else:
                print('Wrong Option!')
    ...
    def vec_predict(self):
        ...
        if validate:
            ...
            #Predict it
            if(self.kernel_variable.get()=='RBF'):
                y_pred=self.scalery.inverse_transform((svrRbf.predict(self.scalerx.transform(vec))))
                self.cal_value.set(y_pred)
            if(self.kernel_variable.get()=='Linear'):
                y_pred=self.scalery.inverse_transform((svrLin.predict(self.scalerx.transform(vec))))
                self.cal_value.set(y_pred)
            if(self.kernel_variable.get()=='Polynomial'):
                y_pred=self.scalery.inverse_transform((svrPoly.predict(self.scalerx.transform(vec))))
                self.cal_value.set(y_pred)
