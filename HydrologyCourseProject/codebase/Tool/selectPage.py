...
class SelectPage(tk.Frame):
    ...
    def initUI(self, parent, controller):
        ...
        TaskSixButton = self.customButton(text = "Box Plot Analysis", 
        command =  lambda: controller.show_frame(BoxPlotAnalysis))
        TaskSixButton.place(x =1000, y = 10, width = 160, height = 35)


        TaskSevenButton = self.customButton(text = "GLDAS Analysis", 
        command =  lambda: controller.show_frame(GLDAS))
        TaskSevenButton.place(x =1200, y = 10, width = 160, height = 35)

        TaskEightButton = self.customButton(text = "Drought Index", 
        command =  lambda: controller.show_frame(DroughtIndex))
        TaskEightButton.place(x =1380, y = 10, width = 160, height = 35)
